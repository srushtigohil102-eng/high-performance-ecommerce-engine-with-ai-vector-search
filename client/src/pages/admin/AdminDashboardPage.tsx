...
export default function AdminDashboardPage() {
  ...
  const cards = [
    ...
    { label: 'Low Stock Items', value: stats.lowStockProducts, color: 'bg-amber-50 text-amber-700' },
    { label: 'Total Revenue', value: formatCurrency(stats.totalRevenue), color: 'bg-green-50 text-green-700' },
  ]
  ...
  return (
    <div>
      ...
      <div className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {cards.map((card) => (
          <div key={card.label} className={`rounded-lg border border-gray-200 p-5 ${card.color}`}>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <Link
          to="/admin/products"
          className="flex items-center gap-4 rounded-lg border border-gray-200 p-5 transition hover:border-gray-900 hover:shadow-sm"
        >
